...
GH_ATTEMPTS = 3
# Concurrent `gh` reads for the per-PR view fan-out. Bounded so a wide month cannot
# open one process per PR, and low enough to stay clear of secondary rate limits.
GH_MAX_PARALLEL = 8
...
PHASE_PR_KEY_RE = re.compile(r"^phase_[a-z0-9_-]+_pr$", re.IGNORECASE)
...
FRONTMATTER = load_module(
    "nase_frontmatter_scalar",
    SCRIPT_ROOT / ".claude" / "scripts" / "frontmatter_scalar.py",
)
EFFORT_STATE = load_module(
    "nase_effort_state", SCRIPT_ROOT / ".claude" / "scripts" / "effort-state.py"
)
SECRET = load_module(
    "nase_effort_rollup_secret_scan",
...
def parse_scope(context: Path, repo_filter: str | None) -> list[dict[str, str]]:
    ...
    for line in section_lines(context.read_text(encoding="utf-8"), "Repos"):
        ...
        repos.append(
            {"alias": alias, "owner": owner, "repo": name, "full_name": full_name}
        )
    if not repos:
...
def github_remote(path: Path) -> str | None:
    ...
    match = re.search(
        r"github\.com[:/]([^/]+)/([^/]+?)(?:\.git)?$", remote, re.IGNORECASE
    )
    return f"{match.group(1)}/{match.group(2)}" if match else None


def local_availability(
    scope: list[dict[str, str]], mappings: dict[str, Path]
) -> list[dict[str, str]]:
...
def extract_prs(value: str) -> list[str]:
    ...
    found.extend(
        canonical_pr(*match.groups()) for match in QUALIFIED_PR_RE.finditer(value)
    )
...
def context_prs(text: str) -> list[str]:
    ...
    for line in text.splitlines():
        if EFFORT_STATE.BARE_PR_DENIAL_RE.search(EFFORT_STATE.deemphasize(line)):
            found.extend(
                canonical_pr(*match.groups()) for match in FULL_PR_RE.finditer(line)
            )
        else:
...
def effort_files(root: Path) -> list[tuple[Path, str]]:
    ...
    for path, _ in files:
        if (
            path.is_symlink()
            or not path.is_file()
            or not inside(path.resolve(), root.resolve())
        ):
...
def parse_efforts(root: Path) -> list[dict[str, Any]]:
    ...
    for path, location in effort_files(root):
        ...
        if normalized_status not in VALID_EFFORT_STATUSES:
            ...
            ACTIVE_EFFORT_STATUSES if location == "active" else DONE_EFFORT_STATUSES
        ):
            ...
            *sorted(
                {
                    line.split(":", 1)[0]
                    for line in lines
                    if PHASE_PR_KEY_RE.fullmatch(line.split(":", 1)[0])
                }
            ),
        ]:
        ...
        if (
            any(re.match(r"^prs\s*:", line, re.IGNORECASE) for line in lines)
            and not prs_valid
        ):
...
def efforts_for_scope(
    efforts: list[dict[str, Any]], scope: list[dict[str, str]]
) -> list[dict[str, Any]]:
    ...
    for effort in efforts:
        ...
        if (
            repo_alias in aliases
            or repo_alias.lower() in repositories
            or delivery_repos & repositories
        ):
...
def run_json(
    ...
    for attempt in range(1, attempts + 1):
        ...
        if completed.returncode == 0:
            try:
                return CommandResult(
                    "complete", json.loads(completed.stdout), raw, attempt, None
                )
        ...
        if last_category == "rate-limited":
            if wait is None or wait > 60:
                return CommandResult(
                    "partial", None, raw, attempt, "rate-limit-wait-too-long"
                )
            if attempt < attempts:
                time.sleep(wait)
                continue
    ...
    return CommandResult(
        "partial", None, last_raw, attempts, f"retry-exhausted-{last_category}"
    )


def fetch_concurrently(plan: dict[str, list[str]]) -> dict[str, CommandResult]:
    """Run independent `gh` reads concurrently, keyed by the caller's own identifier.
    ...
    with ThreadPoolExecutor(max_workers=min(GH_MAX_PARALLEL, len(plan))) as pool:
        pending = {key: pool.submit(run_json, args) for key, args in plan.items()}
        ...
        self.captures.append(
            {
                ...
                "record_count": len(outcome.data)
                if isinstance(outcome.data, list)
                else (1 if isinstance(outcome.data, dict) else 0),
            }
        )
...
def search_candidates(
    ...
    for item in outcome.data:
        ...
        if (
            not match
            or f"{match.group(1)}/{match.group(2)}".lower() != repo["full_name"].lower()
        ):
            schema_error = True
            continue
        urls[pr_key(url)] = url
    ...
    right, right_gaps = search_candidates(
        collector, repo, author, midpoint + timedelta(days=1), end
    )
    left.update(right)
    return left, left_gaps + right_gaps


def collect_pr_views(
    collector: Collector, candidates: dict[str, str]
...
def classify(
    ...
    for effort in efforts:
        for role, field in (
            ...
            for url in effort[field]:
                memberships.setdefault(pr_key(url), {}).setdefault(role, set()).add(
                    effort["slug"]
                )
    ...
    for effort in efforts:
        ...
        intersects_terminal_month = effort["location"] in {
            "done",
            "archive",
        } and effort["source_mtime_month"] == start.strftime("%Y-%m")
        active_tail = effort["location"] == "active" and effort["status"] in {
            "awaiting-deploy",
            "merged",
            "completed",
        }
    ...
    for key in sorted(all_keys):
        ...
        role = next(
            (candidate for candidate in priority if candidate in roles),
            "untracked-candidate",
        )
        linked = sorted(set().union(*roles.values())) if roles else []
        countable_efforts = {
            effort["slug"] for effort in efforts if effort["countable"]
        }
        ...
        countable = (
            role == "delivery"
            and merged_in_month
            and bool(set(linked) & countable_efforts)
        )
        if base:
            record = dict(base)
        else:
            ...
            record = {
                "id": f"pr:{key}",
                "url": canonical_pr(owner, repo, number),
                "owner": owner,
                "repo": repo,
                "number": int(number),
                "title": None,
        ...
        record.update(
            {
                ...
                if countable
                    ...
                    "untracked-candidate"
                    if role == "untracked-candidate"
                    else "non-delivery-role"
                    if role != "delivery"
                    else "not-merged-in-month"
                    if not merged_in_month
                    else "no-countable-effort"
                ),
            }
        )
        pr_records.append(record)
    totals = {
        "delivered_efforts": len(
            {effort["slug"] for effort in efforts if effort["countable"]}
        ),
        "merged_delivery_prs_in_month": sum(
            record["countable"] for record in pr_records
        ),
        "tracked_external_efforts": sum(
            effort["bucket"] == "tracked-external" for effort in efforts
        ),
...
def inventory_capture(root: Path, month: str, bundle: Path) -> dict[str, Any]:
    ...
    directories = [
        root / "workspace" / "efforts" / "done",
        root / "workspace" / "efforts" / "archive" / month[:4],
    ]
    for directory in directories:
        ...
        commands.append(
            [
                "bash",
                ".claude/scripts/month-efforts.sh",
                month,
...
def build_evidence(
    ...
    classification_gaps = [
        f"{effort['slug']}:classification-error"
        for effort in efforts
        if effort["classification_errors"]
    ]
    all_gaps = sorted(set(gaps + classification_gaps))
    coverage = {
        "status": "partial" if all_gaps else "complete-for-declared-sources",
        "gaps": all_gaps,
    }
    return {
        "schema_version": 1,
...
def collect(args: argparse.Namespace) -> int:
    ...
    for repo in scope:
        ...
        discovered, repo_gaps = search_candidates(
            collector, repo, author, start.date(), (end - timedelta(days=1)).date()
        )
        ...
        source_coverage.append(
            {
                ...
                "reasons": sorted(
                    set(repo_gaps + ([] if rate_reason is None else [rate_reason]))
                ),
            }
        )
    ...
    print(
        ...
        if args.format == "json"
        else f"run_id={run['run_id']} coverage={evidence['coverage']['status']}"
    )
    return 0


def capture_data(
    bundle: Path, entries: list[dict[str, Any]], started: datetime, finished: datetime
) -> dict[str, Any]:
    values: dict[str, Any] = {}
    for entry in entries:
        ...
        if (
            collected < started
            or collected > finished
            or collected > datetime.now(timezone.utc) + timedelta(seconds=5)
        ):
            raise EvidenceError(
                f"capture timestamp outside collection interval: {entry['source_id']}"
            )
...
def validate_capture_contract(
    entries: list[dict[str, Any]], run: dict[str, Any]
) -> None:
    ...
    expected_sources = {
        source.get("alias"): source.get("account") for source in raw_sources
    }
    ...
    for entry in entries:
        ...
        if kind == "effort-inventory":
            if (
                seen_inventory
                or source_id != "effort-inventory"
                or capture_path != "captures/effort-inventory.txt"
            ):
                raise EvidenceError("invalid effort inventory identity")
            if (
                entry.get("logical_window") != {"month": run["month"]}
        ...
        if kind == "github-rate":
            ...
            match = re.fullmatch(
                r"github-search:([^:]+):([^:]+):([0-9-]+\.\.[0-9-]+)", source_id
            )
            ...
            if (
                ...
                or author != expected_sources.get(alias)
                or command != expected
            ):
            ...
            expected = [
                ...
                number,
                "--repo",
                command[5] if len(command) > 5 else "",
                "--json",
                "number,title,state,mergedAt,author,url",
            ]
...
def reconstruct_discovery(
    ...
    if set(searches) != visited:
        raise EvidenceError(
            "unexpected search capture outside canonical month partition"
        )
    return sources, search_keys


def reconstruct_live_prs(
    entries: list[dict[str, Any]], values: dict[str, Any]
) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    for entry in entries:
        ...
        try:
            ...
            if not isinstance(value["title"], str) or not isinstance(
                value["state"], str
            ):
...
def contract_tokens(evidence: dict[str, Any], evidence_sha: str) -> list[str]:
    ...
    required.extend(
        effort["id"] for effort in evidence["efforts"] if effort["countable"]
    )
    return required
...
def markdown_errors(
    markdown: Path, evidence: dict[str, Any], evidence_sha: str
) -> list[str]:
...
def html_errors(
    document: Path, evidence: dict[str, Any], evidence_sha: str
) -> list[str]:
    ...
    for item in contract_tokens(evidence, evidence_sha):
        ...
        if item.startswith("https://"):
            found = any(
                token_present(item, href) for href in probe.link_targets
...
def validate(args: argparse.Namespace) -> int:
    ...
    if (
        ...
        or manifest.name != "evidence.json"
    ):
    ...
    if (
        run.get("run_id") != evidence.get("run_id")
        or run.get("month") != args.month
        or evidence.get("month") != args.month
    ):
        raise EvidenceError("foreign run ID or month")
    if (
        run.get("measurement_basis_version") != BASIS_VERSION
        or evidence.get("measurement_basis_version") != BASIS_VERSION
    ):
    ...
    current_scope = parse_scope(
        required_file(root, "workspace/context.md"), run["scope"].get("repo_filter")
    )
    if current_scope != run["scope"].get("expected_repos"):
        raise EvidenceError("scope drift")
    expected_accounts = {
        repo["alias"]: discovery_account(repo, config) for repo in current_scope
    }
    recorded_accounts = {
    ...
    current_local = local_availability(
        current_scope,
        local_path_values(
            required_file(root, ".local-paths"),
            {repo["alias"] for repo in current_scope},
        ),
    )
    ...
    mismatches = [
        field for field in compared if evidence.get(field) != derived.get(field)
    ]
    ...
    if args.markdown:
        ...
        if (
            ...
            or not inside(markdown.resolve(), bundle)
        ):
    ...
    if args.html:
        ...
        if (
            ...
            or not inside(document.resolve(), bundle)
        ):
    ...
    print(
        ...
        if args.format == "json"
        else f"ok={str(validation['ok']).lower()} evidence_sha256={evidence_sha}"
    )
