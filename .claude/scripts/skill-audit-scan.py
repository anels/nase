...
def rule(
    category: int, name: str, severity: str, label: str, pattern: str, reason: str
) -> Rule:
    return Rule(
        category, name, severity, label, re.compile(pattern, re.IGNORECASE), reason
    )
...
RULES = (
    rule(
        ...
        r"\brm\s+(?:(?:--recursive|-[A-Za-z]*r[A-Za-z]*)(?:\s+--?[A-Za-z-]+)*)\s+(?:--\s+)?[\"']?(?:/|~|\.\.)",
        "can delete files outside the workspace",
    ),
    rule(
        1,
        "Command Injection",
        "FAIL",
        "piped download",
        r"\b(?:curl|wget)\b[^\n|]*\|\s*[\"']?(?:(?:/[A-Za-z0-9._-]+)*/)?(?:bash|sh|python(?:3)?)\b",
        "downloads and executes untrusted content",
    ),
    rule(
        1,
        "Command Injection",
        ...
        r"\b(?:(?:eval|exec)\b[^\n]*\$(?:\{)?(?:ARGUMENTS|USER_INPUT|INPUT|URL|PATH)\b|source\s+[\"']?\$(?:\{)?(?:ARGUMENTS|USER_INPUT|INPUT|URL|PATH)\b)",
        "executes dynamically supplied input",
    ),
    rule(
        1,
        "Command Injection",
        ...
        r"\bbase64\s+(?:-[^| ]*d|--decode)\b[^|]*\|\s*[\"']?(?:(?:/[A-Za-z0-9._-]+)*/)?(?:bash|sh|python(?:3)?)\b",
        "decodes and executes hidden content",
    ),
    rule(
        1,
        "Command Injection",
    ...
    rule(
        ...
        "Data Exfiltration",
        "FAIL",
        "sensitive data network send",
        r"\bbase64\b[^\n]*(?:\.env|\.ssh|credential|secret|token)[^\n]*(?:curl|wget|http)",
        "encodes sensitive data for a network request",
    ),
    rule(
        2,
        "Data Exfiltration",
        "FAIL",
        "workspace copy outside root",
        r"\b(?:cp|mv|rsync|tee)\b[^\n]*(?:workspace/|\.claude/)[^\n]*(?:/tmp/|~/Desktop/|/[A-Za-z])",
        "copies workspace content outside the workspace",
    ),
    rule(
        2,
        "Data Exfiltration",
        "FAIL",
        "workspace redirection outside root",
        r"\b(?:cat|base64)\b[^\n]*(?:workspace/|\.claude/)[^\n]*>{1,2}\s*[\"']?(?:/|~)",
        "writes workspace content outside the workspace",
    ),
    rule(
        2,
        "Data Exfiltration",
    ...
    rule(
        ...
        "WARN",
        "role reassignment",
        r"\b(?:you are now|from this point on,?\s+you are)\b",
        "attempts to reassign the model role",
    ),
    rule(
        3,
        "Prompt Injection",
        "WARN",
        "rules bypass",
        r"\bdo not follow\b[^\n]*\b(?:CLAUDE\.md|rules|instructions)\b",
        "attempts to bypass repository rules",
    ),
    rule(
        3,
        "Prompt Injection",
        ...
        r"(?:\b(?:new\s+)?system prompt\s*:|<system>)",
        "introduces a competing system prompt",
    ),
    rule(
        3,
        "Prompt Injection",
        "WARN",
        "hidden behavior",
        r"\b(?:do not tell|hide this from)\s+the user\b",
        "asks the agent to conceal behavior",
    ),
    rule(
        3,
        "Prompt Injection",
        "WARN",
        "safety bypass",
        r"\b(?:disable|bypass|skip)\b[^\n]{0,40}\b(?:safety(?:\s+checks?)?|verification(?:\s+(?:steps?|checks?))?)\b",
    ...
    rule(
        ...
        "FAIL",
        "protected user or system path",
        r"(?:~/(?:\.claude/(?!projects/)|\.ssh/|\.gnupg/|\.aws/|\.kube/)|~/(?:\.bashrc|\.zshrc|\.profile|\.gitconfig)\b|/(?:etc|usr|var)/)",
        "modifies a protected path outside the workspace",
    ),
    rule(
        4,
        "Unsafe File Operations",
        "FAIL",
        "absolute path write",
        r"(?:\d*>{1,2}\s*[\"']?/(?!dev/(?:null|stdout|stderr)\b)[A-Za-z0-9._/-]+|\b(?:touch|mkdir|install|tee|cp|mv|chmod|chown)\b[^\n]*\s[\"']?/[A-Za-z0-9._/-]+)",
        "writes to an absolute path outside the workspace",
    ),
    rule(
        4,
        "Unsafe File Operations",
    ...
    rule(
        ...
        "Supply Chain",
        "WARN",
        "non-standard package source",
        r"\b(?:pip|npm|go|cargo)\s+install\b[^\n]*(?:https?://|--index-url|--registry|--git)",
        "installs code from a non-standard source",
    ),
    rule(
        5,
        "Supply Chain",
        "WARN",
        "clone and execute",
        r"\bgit\s+clone\b[^\n]*(?:&&|;)\s*(?:cd|bash|sh|python|make|\./)",
        "executes code immediately after cloning it",
    ),
    rule(
        5,
        "Supply Chain",
        "WARN",
        "downloaded executable or script",
        r"\bcurl\b(?=[^\n]*https?://)(?=[^\n]*(?:-o|--output|-O)\b)[^\n]*",
    ...
    rule(
        ...
        "WARN",
        "unknown MCP server",
        r"\b(?:add|install|register)\b[^\n]*\bMCP server\b",
        "adds an external tool server",
    ),
    rule(
        6,
        "Credential Exposure",
        "FAIL",
        "API token",
        r"\b(?:sk-(?:proj-)?[A-Za-z0-9_-]{20,}|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{35}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{20,}|xox[baprs]-[A-Za-z0-9-]{10,})\b|\bBearer\s+[A-Za-z0-9._~+/=-]{8,}",
        "contains a hardcoded credential-like token",
    ),
    rule(
        6,
        "Credential Exposure",
        "FAIL",
        "hardcoded secret",
        r"\b(?:[A-Z0-9_]*(?:PASSWORD|PASSWD|PWD|TOKEN|API[_-]?KEY|CLIENT[_-]?SECRET|ACCESS[_-]?TOKEN|REFRESH[_-]?TOKEN|SECRET))\b\s*[:=]\s*[\"']?(?!(?:YOUR_[A-Z0-9_]+|EXAMPLE|DUMMY|PLACEHOLDER|REDACTED|CHANGEME)\b)[A-Za-z0-9._~+/@!#%^&*=-]{8,}",
        "contains a non-placeholder hardcoded secret",
    ),
    rule(
        6,
        "Credential Exposure",
...
def shell_line_continues(line: str) -> bool:
    ...
    return trailing_backslashes % 2 == 1 or bool(
...
def markdown_lines(text: str) -> Iterable[tuple[int, str, str]]:
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        if in_fence:
            context = (
                "non-executable"
                if fence_language in NON_EXECUTABLE_FENCES
                else "executable"
            )
        if context == "executable" and shell_line_continues(line):
            ...
            stripped = line.rstrip()
            continued_line += (stripped.removesuffix("\\")) + " "
            continue
        if continued_line:
...
def explanatory(line: str, context: str, rule_item: Rule, match_start: int) -> bool:
    ...
    if context == "non-executable" and rule_item.category != 6:
        return True
    if rule_item.label == "absolute path write" and re.search(
        r"<[A-Za-z0-9_-]+$", line[:match_start]
    ):
        return True
    if (
    ...
    markers = [
        match.start()
        for regex in (explanation, EXAMPLE_RE)
    ...
    for line_number, line, context in markdown_lines(text):
        for rule_item in RULES:
            ...
            if match and not explanatory(line, context, rule_item, match.start()):
                ...
                if (
                    rule_item.category in PRIVILEGE_CATEGORIES
                    and privilege_line is None
                ):
...
def payload(paths: list[Path], root: Path) -> dict[str, object]:
    ...
    counts = {
        verdict.lower(): sum(item["verdict"] == verdict for item in results)
...
def print_report(data: dict[str, object]) -> None:
    ...
    for result in data["results"]:
        print(
            f"| `{result['file']}` | {result['verdict']} | {len(result['findings'])} |"
        )
    for result in data["results"]:
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "target", nargs="?", default="all", help="file, directory, or all"
    )
