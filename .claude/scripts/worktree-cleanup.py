#!/usr/bin/env python3
"""Verify and safely quarantine a linked Git worktree without force."""
...
def cleanup(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if locked.returncode != 0:
            return retained(
                "could not lock the claimed worktree; preserving it",
                [
                    f"registered-worktree:{claimed}",
                    locked.stderr.strip() or locked.stdout.strip() or "unknown error",
                ],
            )
        ...
        if claimed_dirty:
            return retained(
                f"claimed worktree changed after claim: {claimed}",
                [f"registered-worktree:{claimed}", *claimed_dirty],
            )

        return retained(
            "claimed clean worktree is locked because automatic deletion cannot prove a later write will not occur",
            [f"registered-worktree:{claimed}", f"head:{expected}"],
        )
    finally:
