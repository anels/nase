...
TOOLS: tuple[Tool, ...] = (
    Tool(
        ...
        "rg",
        "ripgrep",
        "repo search, context gathering, sensitive artifact scans",
    ),
    ...
    Tool(
        ...
        "trivy",
        "trivy",
        "dependency, image, filesystem, IaC, and secret scans",
    ),
    Tool(
        "security",
        "gitleaks",
        "gitleaks",
        "gitleaks",
        "secret scanning for repos, diffs, and staged changes",
    ),
    ...
    Tool(
        ...
        "difft",
        "difftastic",
        "syntax-aware diffs with JSON output",
    ),
    Tool(
        "repo",
        "ripgrep-all",
        "rga",
        "ripgrep-all",
        "search inside PDFs, Office docs, and archives",
    ),
    ...
    Tool(
        ...
        "ctags",
        "universal-ctags",
        "symbol inventory and language-aware indexes",
    ),
    ...
    Tool(
        ...
        "duckdb",
        "duckdb",
        "SQL over local CSV, JSON, Parquet, and logs",
    ),
    ...
    Tool(
        ...
        "ccusage",
        "ccusage",
        "coding-agent token and cost summaries",
    ),
...
def print_brew_install(rows: list[dict[str, str | None]]) -> None:
    formulas = sorted(
        {str(row["brew"]) for row in rows if row["status"] == "missing" and row["brew"]}
    )
    if formulas:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        choices=GROUPS,
        help="Tool group to report; repeatable",
    )
    parser.add_argument("--all", action="store_true", help="Report every known tool")
    parser.add_argument("--format", choices=("table", "json"), default="table")
    parser.add_argument(
        "--missing", action="store_true", help="Show only missing tools"
    )
    parser.add_argument(
        "--install",
        choices=("brew",),
        help="Print one install command for missing tools",
    )
