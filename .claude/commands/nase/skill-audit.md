...
`allowed-tools` in frontmatter only pre-approves matching tools and does not block anything else. Real enforcement comes from permission deny rules, `--disallowedTools` / SDK `disallowed_tools`, sandboxing, or PreToolUse hooks, and applies to Categories 1, 2, 4, 5, and 7.

Read `.claude/docs/skill-permission-profiles.md` when a Category 7 finding needs a recommendation. It owns the settings shape, the per-purpose enforcement profiles, and the ready-to-paste snippet the report should carry.
