...
- **Local workspace skill integrity**: follows `.claude/docs/workspace-write-guard.md`'s generated integrity-manifest exception. When `workspace/skills/` exists, run `python3 .claude/scripts/workspace-skill-integrity.py --root . check`. Pass only when the reviewed local source manifest and generated wrapper match, and no legacy generated native mirror remains. A missing manifest, source-wrapper drift, or legacy mirror is a FAIL. After reviewing intentional local-source edits, restart the session hook to repair wrappers and remove legacy mirrors, then run `write-manifest` to establish the new local baseline. The helper refuses to refresh the baseline while any check fails.
