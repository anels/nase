...
- **Structural skeleton** - table column headers **and fixed row labels** (Sense Check pillar names, verification-matrix layer labels), code blocks, file paths, line numbers, symbol/identifier names, **issue and ticket keys (`IN-####`) and any other ID**, daily-log entries, JSON keys/values, command output → keep English. This holds inside a narrative cell too: an evidence cell is written in `conversation:` but the `IN-####` it cites is not translated.
- **GitHub-bound drafts and the posted review** → write in the `output:` value, not `conversation:` and not English-by-default. Paths, symbols, and `path:line` inside them stay English like any skeleton.
...
At Step 6, read `.claude/docs/discuss-pr-output.md` once. Follow its exact output order, scorecard, severity/disposition grouping, automatic additional deep dives, draft decision, gated review submission, completion message, KB auto-write, error handling, notes, and daily log.
...
Steps 6, 6.5, and 7 run in one turn. The findings report is the setup for the draft question, not the deliverable, so the turn ends on Step 7's `AskUserQuestion` - never on the report itself. Stopping after the report reads to the user as an abandoned review and costs them a round trip to say "continue".
