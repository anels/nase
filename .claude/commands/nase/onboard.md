...
- **Already-captured dedup.** A repo can show N commits since the KB date while the KB was actually written later than the domain-map claimed (or the commits are test/chore noise). Before persisting any candidate PR#/feature, grep the existing KB file for it; write only what is not already captured. A refresh that finds everything already present is a correct no-op — do not re-append a feature the KB already documents just because it postdates the recorded date.
