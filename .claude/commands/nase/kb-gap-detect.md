...
For each gap, produce a draft in the dated entry format `/nase:kb-update` defines, including its `**Tags:**` vocabulary and field-omission rules. Two fields carry gap-specific content: `**Details:**` holds the condensed snippets, and `**Source:**` names the authoritative repo, documentation, ticket, or runbook the claim was verified against.
...
A title line carrying the report name and `{today}`, a provenance line naming this
command and the `{START}..{END}` range, a counts line
(`{D} log days, {M} hits, {K} clusters, {G} gaps, {T} thin-coverage`), then one `##`
section per gap, `---` separated:

```
## Gap {n}: {topic}  [recurrence: {N}, confidence: {high|medium|low}]
**Markers:** {marker mix}
**Evidence:**
- {file}:{line} — "{snippet}"
**KB target:** `{path}`
