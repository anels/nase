...
- Effort PR sweep: `.claude/scripts/effort-pr-sweep.py` batch-reads live PR state for every active effort and audits the two mechanical blind spots in `effort-lifecycle.md → Classifier Blind Spots` - delivery PRs made invisible by a non-canonical lifecycle row label, and merge commits a later revert commit names. `/nase:efforts` Step 3 runs it; `tests/scripts/test-effort-pr-sweep.sh` covers the offline label audit and the exclusion hints.
