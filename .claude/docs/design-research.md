...
- **Metric-ceiling feasibility (go/no-go).** If the build's value rests on hitting a target metric (e.g. "≥10% X reduction", "≥N% auto-fix rate"), first compute that metric's observable **CEILING** from existing history with a throwaway, deliberately over-inclusive classifier — count everything that could *possibly* qualify. If even that generous ceiling is below target, kill the thesis before building the feature or its trial harness. Do this at thesis-definition time, not after the action layer exists (a 799-comment corpus whose generous ceiling was 4.8% against a 10% target should never have reached implementation).
