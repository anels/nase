...
- **Size** - if the diff crosses a `size` threshold that mandates `## How to Review`, ensure that section is filled (this composes with fsd's own Phase 6.1 final diff-size guardrail; the gate profile only adds the repo-specific *requirement*, not a second size opinion).
