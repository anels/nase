...
1. If `$ARGUMENTS` is empty or does not contain a GitHub PR URL: output `Usage: /{skill-name} <PR-URL>` and stop. (Skills may override this to ask the user interactively via `AskUserQuestion` instead — see individual skill instructions.)
