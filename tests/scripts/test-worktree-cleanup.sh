...
linked_worktree_is_locked() {
  git -C "$CASE_REPO" worktree list --porcelain |
    awk -v target="$1" '
      /^worktree / { active = substr($0, 10) == target; next }
      active && /^locked( |$)/ { found = 1 }
      END { exit !found }
    '
}
...
make_git_wrapper() {
  ...
  printf '%s\n' \
    ...
    '  fi' \
    'fi' \
    'if [[ "${NASE_TEST_MODE:-}" == post-claim-error && " $* " == *" submodule status "* ]]; then' \
    '  count=0' \
    '  [[ ! -f "$NASE_TEST_COUNT_FILE" ]] || count=$(<"$NASE_TEST_COUNT_FILE")' \
    '  count=$((count + 1))' \
    '  printf "%s\\n" "$count" >"$NASE_TEST_COUNT_FILE"' \
    '  if [[ $count -eq 2 ]]; then printf "injected submodule failure\\n" >&2; exit 42; fi' \
    'fi' \
...
new_case clean
expect_rc "clean exact pushed worktree is claimed and retained" 3 run_helper
claimed_path=$(linked_worktree_path)
[[ -d "$claimed_path" && "$claimed_path" != "$CASE_WT" ]] || fail "clean worktree was not claimed"
[[ ! -e "$CASE_WT" ]] || fail "clean worktree original path still exists"
linked_worktree_is_locked "$claimed_path" || fail "clean claimed worktree is not locked"
CASE_WT=$claimed_path
expect_rc "locked claimed worktree is not claimed again" 3 run_helper
[[ $(linked_worktree_path) == "$claimed_path" ]] || fail "locked claimed worktree moved again"
...
new_case post-claim-error
rm -f "$TMP/ls-remote-count"
TEST_MODE=post-claim-error
expect_rc "post-claim inspection error removes safety ref" 2 run_helper_injected
claimed_path=$(linked_worktree_path)
[[ -d "$claimed_path" && "$claimed_path" != "$CASE_WT" ]] || fail "post-claim error lost worktree"
[[ -z $(git -C "$CASE_REPO" for-each-ref refs/nase/worktree-cleanup/) ]] || fail "safety ref remained after post-claim error"
...
new_case detached
git -C "$CASE_REPO" worktree remove "$(linked_worktree_path)"
...
expect_rc "detached worktree with exact remote SHA is claimed and retained" 3 python3 "$HELPER" \
  --repo "$CASE_REPO" --worktree "$CASE_WT" --remote origin \
  --remote-ref refs/heads/detached-clean --expected-head "$CASE_HEAD"
claimed_path=$(linked_worktree_path)
[[ -d "$claimed_path" && "$claimed_path" != "$CASE_WT" ]] || fail "detached worktree was not claimed"
[[ ! -e "$CASE_WT" ]] || fail "detached worktree original path still exists"
linked_worktree_is_locked "$claimed_path" || fail "detached claimed worktree is not locked"
...
if rg -n -U '"worktree",\s*"remove"' "$HELPER" >"$TMP/remove-call"; then
  fail "helper still automatically removes a worktree"
else
...
if rg -n 'Worktree:[[:space:]]+cleaned up' "$ROOT/.claude/commands/nase/fsd.md" >"$TMP/fsd-worktree"; then
  ...
  "$ROOT/.claude/commands/nase/fsd.md"; then
