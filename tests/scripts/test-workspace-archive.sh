...
# Unknown headings are boundaries, not part of the preceding promoted lesson.
fixture = make_root()
source = fixture / "workspace/tasks/lessons.md"
source.write_text(
    "# Lessons\n\n"
    + lesson("promoted body")
    + "## Follow-up\n\nkeep this live\n",
    encoding="utf-8",
)
module.rotate_lessons(fixture)
assert "promoted body" in (fixture / "workspace/tasks/lessons-archive.md").read_text(
    encoding="utf-8"
)
assert "## Follow-up\n\nkeep this live\n" in source.read_text(encoding="utf-8")
print("PASS  unknown heading stays live after promoted lesson archive")
...
def fail_source(path, content, validate, snapshot):
    if path == source:
...
def make_pending(kind):
    ...
    def fail_source_replace(path, content, validate, snapshot):
        if path == source:
            raise OSError("injected source failure")
        return real_atomic_replace(path, content, validate, snapshot)
...
# Source parents must be lexical directories before source bytes reach a journal.
fixture = make_root()
outside = fixture / "outside-lessons"
outside.mkdir()
outside_source = outside / "lessons.md"
outside_source.write_text("# Lessons\n\n" + lesson(), encoding="utf-8")
source_parent = fixture / "workspace/tasks"
source_parent.rmdir()
...
try:
    ...
    raise AssertionError("symlinked lessons parent was accepted")
assert outside_source.read_text(encoding="utf-8") == "# Lessons\n\n" + lesson()
assert not module.journal_path(fixture, "lessons").exists()

fixture = make_root()
outside = fixture / "outside-trends"
outside.mkdir()
outside_source = outside / "tech-trends.md"
outside_source.write_text("# Tech Trends\n\n" + trend("2020"), encoding="utf-8")
source_parent = fixture / "workspace/kb/general"
source_parent.rmdir()
source_parent.symlink_to(outside, target_is_directory=True)
try:
    ...
    raise AssertionError("symlinked tech trends parent was accepted")
assert outside_source.read_text(encoding="utf-8") == "# Tech Trends\n\n" + trend("2020")
assert not module.journal_path(fixture, "tech-trends").exists()
print("PASS  source parent symlinks never reach an archive journal")

# Archive changes between snapshot and publish must leave source and journal intact.
for archive_exists in (False, True):
    ...
    def inject_archive_change(path, content, validate, snapshot):
        if path == archive:
            archive.write_text("CONCURRENT EDIT\n", encoding="utf-8")
        return real_replace(path, content, validate, snapshot)

    with mock.patch.object(module, "atomic_replace", side_effect=inject_archive_change):
        try:
            module.rotate_lessons(fixture)
        except module.ArchiveError:
            pass
        else:
            raise AssertionError("archive change was overwritten")
    assert source.read_bytes() == source_before
    assert archive.read_text(encoding="utf-8") == "CONCURRENT EDIT\n"
    assert module.journal_path(fixture, "lessons").exists()
print("PASS  archive drift preserves source and journal")

# A late archive update must win over the archive publisher, without source cleanup.
fixture = make_root()
source = fixture / "workspace/tasks/lessons.md"
source.write_text("# Lessons\n\n" + lesson(), encoding="utf-8")
source_before = source.read_bytes()
...
def change_archive_before_claim(old, new):
    global changed
    if old == archive and ".backup-" in Path(new).name and not changed:
        changed = True
        archive.write_text("CONCURRENT ARCHIVE\n", encoding="utf-8")
    return real_rename(old, new)


with mock.patch.object(module.os, "rename", side_effect=change_archive_before_claim):
    try:
        module.rotate_lessons(fixture)
    except module.ArchiveError:
        pass
    else:
        raise AssertionError("late archive update was overwritten")
assert archive.read_text(encoding="utf-8") == "CONCURRENT ARCHIVE\n"
assert source.read_bytes() == source_before
assert module.journal_path(fixture, "lessons").exists()
print("PASS  late archive update preserves source and journal")

# A late source update must win over source cleanup after archive publication.
fixture = make_root()
source = fixture / "workspace/tasks/lessons.md"
source.write_text("# Lessons\n\n" + lesson(), encoding="utf-8")
archive = fixture / "workspace/tasks/lessons-archive.md"
...
def change_source_before_claim(old, new):
    global changed
    if old == source and ".backup-" in Path(new).name and not changed:
        changed = True
        source.write_text("CONCURRENT SOURCE\n", encoding="utf-8")
    return real_rename(old, new)


with mock.patch.object(module.os, "rename", side_effect=change_source_before_claim):
    try:
        module.rotate_lessons(fixture)
    except module.ArchiveError:
        pass
    else:
        raise AssertionError("late source update was overwritten")
assert source.read_text(encoding="utf-8") == "CONCURRENT SOURCE\n"
assert module.MARKER_PREFIX in archive.read_text(encoding="utf-8")
assert module.journal_path(fixture, "lessons").exists()
print("PASS  late source update preserves archive and journal")

# A source backup left by a stopped publisher is restored before a journal retry.
fixture = make_root()
source = fixture / "workspace/tasks/lessons.md"
source.write_text("# Lessons\n\n" + lesson(), encoding="utf-8")
real_replace = module.atomic_replace
...
def fail_source_publish(path, content, validate, snapshot):
    if path == source:
        raise OSError("source publish stopped")
    return real_replace(path, content, validate, snapshot)


with mock.patch.object(module, "atomic_replace", side_effect=fail_source_publish):
    try:
        module.rotate_lessons(fixture)
    except OSError:
        pass
backup = source.with_name(f".{source.name}.backup-interrupted")
os.rename(source, backup)
assert not source.exists()
module.rotate_lessons(fixture)
assert not source.exists() or "## debugging" not in source.read_text(encoding="utf-8")
assert not backup.exists()
assert not module.journal_path(fixture, "lessons").exists()
print("PASS  orphaned source backup is recovered before journal retry")

# A source-parent swap after archive commit must not write through a symlink.
fixture = make_root()
source = fixture / "workspace/tasks/lessons.md"
source.write_text("# Lessons\n\n" + lesson(), encoding="utf-8")
source_bytes = source.read_bytes()
source_mtime_ns = source.stat().st_mtime_ns
outside = fixture / "outside-source"
outside.mkdir()
outside_source = outside / "lessons.md"
outside_source.write_bytes(source_bytes)
os.utime(outside_source, ns=(source_mtime_ns, source_mtime_ns))
original_parent = source.parent
saved_parent = fixture / "workspace/tasks-original"
real_commit_archives = module.commit_archives


def swap_source_parent(root, data):
    real_commit_archives(root, data)
    original_parent.rename(saved_parent)
    original_parent.symlink_to(outside, target_is_directory=True)


with mock.patch.object(module, "commit_archives", side_effect=swap_source_parent):
    try:
        module.rotate_lessons(fixture)
    except module.ArchiveError:
        pass
    else:
        raise AssertionError("source parent swap was accepted")
assert outside_source.read_bytes() == source_bytes
assert (saved_parent / "lessons.md").read_bytes() == source_bytes
assert module.journal_path(fixture, "lessons").exists()
print("PASS  source parent swap preserves external content and journal")
...
def fail_second_year(path, content, validate, snapshot):
