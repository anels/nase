...
| `PreCompact` | Before context compaction | `pre-compact-archive.sh` | Rotates `workspace/tasks/lessons.md` entries marked `> Promoted →` and older than 90 days into `lessons-archive.md` when the file exceeds 80 KB; archive content is fsynced before source cleanup and transaction markers make retries idempotent; moves `workspace/efforts/done/*.md` older than 60 days with no-clobber guarded moves |
