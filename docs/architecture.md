...
`git worktree move`, then calls ordinary `git worktree remove` without
`--force`. A refusal preserves the claimed worktree. A temporary local ref pins
detached HEAD through the claim.
