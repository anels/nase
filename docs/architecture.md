...
|------|-------|--------|----------|
| `SessionStart` | New Claude Code session | `session-start.sh` | Creates `workspace/logs/YYYY-MM-DD.md` if missing; alerts if last backup had an error or target unreachable; archives tech-digest entries older than 30 days through the shared mutation lock and an fsynced per-source transaction journal under `.nase-locks/`; pending journals bypass the normal source-exists gate; retry validates the exact journal removal plan against the original source before completing all yearly archives and publishing the bound after-image; then the journal is deleted and its directory fsynced; syncs `workspace/skills/*.md` into `/nase:workspace:*` command wrappers and removes legacy generated native mirrors; `/nase:doctor` checks source-wrapper integrity plus mirror cleanup against the ignored manifest; suggests `/nase:reflect` if commits exist for today |
...
| `PreToolUse:slack_send_message`, `slack_schedule_message` | Before a direct or scheduled Slack send | `slack-send-guard.sh` | Blocks both; a scheduled send is still a send. Use draft messages instead |
| `PreToolUse:slack_send_message_draft` | Before a Slack draft is created | `prose-lint-guard.sh` | Runs `prose-lint.py` over the draft body. Blocks on mechanical gate findings (embed link, bare URL ending a line whose next line is non-empty, literal bullet character, emoji-led bullet, bold-term definition list, em dash) and on a register-marker count above the threshold. The Slack gates restate `.claude/docs/slack-draft-style.md` Formatting Mechanics, which stays their source of truth. Fails open when `jq`, `python3`, or the linter is missing. Disable per-run with `NASE_PROSE_LINT=0`. Never a verdict on authorship - see `.claude/docs/plain-writing-guard.md` Part 5 |
| `PreToolUse:Jira mutations` | Before Jira writes | `jira-write-guard.sh` | Requires a fresh `workspace/.jira-write-token`; publication, validation, atomic consumption, and audit logging share the repo-wide workspace mutation lock. Covers both comment tool generations (`addCommentToJiraIssue`, `addOrEditJiraIssueComment`) |
| `PreToolUse:Confluence writes` | Before Confluence content writes | `confluence-size-guard.sh` | Blocks bodies over 70000 bytes to avoid truncation/partial writes, and blocks an unset or unaccepted `contentFormat`. Matches both MCP tool generations: `create/updateConfluencePage` (top-level `contentFormat`, `adf`/`html`/`markdown`) and `create/updateConfluenceContent` (`body:{format,value}`, `html`/`markdown` plus the non-doc `svg`/`csv`/`url`) |
| `PreToolUse:executeWrite`, `executeDestructive` | Before an Atlassian generic write runner | `atlassian-generic-write-guard.sh` | Blocks both. The generic runner can perform any catalog write under an opaque `{name, cloudId, inputs}` payload, so it bypasses the Jira token gate and the Confluence format/size cap. Read-only `discover` and `executeRead` are not matched |
| `PreCompact` | Before context compaction | `pre-compact-archive.sh` | Rotates `workspace/tasks/lessons.md` entries marked `> Promoted →` and older than 90 days into `lessons-archive.md` when the file exceeds 80 KB; a pending journal bypasses the size gate; the per-source journal is fsynced before archive commits, retry reuses its original markers and exact validated after-image, and journal removal follows source cleanup; archive and journal paths must remain lexical regular files under safe repository parents; moves `workspace/efforts/done/*.md` older than 60 days with no-clobber guarded moves |
...
| `PostToolUse:Edit\|Write` | After editing/writing `.py` files | `post-edit-ruff.sh` | Runs `ruff check` on the edited file and returns exit 2 with diagnostics when ruff fails. Ruff discovers its config from the edited file's directory upward, so an edit in another repo is judged by that repo's rules. Exits 0 when ruff is not installed. |
